...
module.exports = (articles) ->
  ...
  projectSearchCollection = projectCollection.createLiveChildCollection()
    ...
    .setFilter('search', (model, searchString) ->
      ...
      pass = searchRegex.test(model.get('body_token')) ||
             searchRegex.test(model.get('title_token'))
      return pass
    )
    .query()
  ...
  return (terms, callback) ->
    ...
    weightedResults = _.sortBy results, (result) ->
      if result.title_token.indexOf(tokenizedTerms) > -1
        0
    highlightedResults = highlighter(weightedResults, terms)
